...
class User extends Model{
    ...
    public function index(){
        $query = $this->db->query('SELECT `tbl_users`.`userID`, `firstName`, `lastName`, `email`, `role`, `password`, `joinDate`, `isDeleted`, `blockedID` FROM `tbl_users` LEFT JOIN `tbl_blockedusers` ON `tbl_users`.`userID` = `tbl_blockedusers`.`userID` ');
